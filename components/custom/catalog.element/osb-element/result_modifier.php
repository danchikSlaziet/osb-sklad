...
if (defined('BX_COMP_MANAGED_CACHE') && is_object($GLOBALS['CACHE_MANAGER']))
{
    ...
    {
        $GLOBALS['CACHE_MANAGER']->RegisterTag('elem_detail_tag'.SITE_ID);
    }
}
...
//        "ID" => $arResult['ID'],
    ...
    if (CModule::IncludeModule("mottex.pricelock")) {
        // выполним его метод
        $pricelock = Mottex\PriceLock\Main::getOption();}
    else{
        $arResult['PriceLock'] = 'Ошибка подключения PriceLock';
    }
    ...
    while ($ob = $res->fetch()) {
        ...
        $price = (int) $ob["PRICE_" . $arResult['ITEM_PRICES'][0]['PRICE_TYPE_ID']];

        if ($pricelock === 'Y' && $price> 0) {
            $ob['QUANTITY'] += rand(450, 550);
        }

        if ($ob['QUANTITY'] === '0' || $price <= 0) $ob['AVAILABLE'] = 'disabled';
        $arResult['ITEMS'][] = $ob;
    }
}
