...
//    debug($arResult);
    ...
    <div class="tab__reviews-buttons">
        <a href="/s1<?=$arResult['LIST_PAGE_URL']?>-<?=$arResult['ID']?>/" class="tab__reviews-btn dark">Оставить отзыв</a>
        <?php if (!empty($arResult['REVIEWS'])): ?>
            <a href="/s1<?=$arResult['LIST_PAGE_URL']?>-<?=$arResult['ID']?>/" class="tab__reviews-btn light">Читать все отзывы</a>
        <? endif; ?>
    </div>
    <?php
    return ob_get_clean();
